...
The expanded audit through **15 September 10:16:49.139 UTC+3** contains **157 failed Codex attempts in its rolling 48-hour window** (123 overload, 30 request-processing errors with request IDs, 4 connection/timeout), with **11 Codex GUI turns ending in error**. Since the fixed original Git baseline `4a398261`, totals are **178 failed attempts and 13 GUI error turns**. The appended logs add **60 failed attempts, all Astra, and eight GUI error turns** relative to the preceding log boundary. Retries count separately; mirrors count once. [Expanded report](https://github.com/gv1983us-commits/codex-provider-incident/blob/main/docs/LOG-EXTENSION-2026-09-15-1016.md) · [New ledger and source hashes](https://github.com/gv1983us-commits/codex-provider-incident/blob/main/evidence/log-extension-20260915T071649Z.json).

The [12 September recurrence](https://github.com/gv1983us-commits/codex-provider-incident/issues/1#issuecomment-5643482050) and this update supersede the earlier resolution claim. Successful runs document temporary continuation; sustained reliable operation remains unresolved. The formerly pending series is now confirmed exhausted at 08:33:21 UTC+3. Expanded logs also show near-simultaneous errors and a fresh session ending in overload. Per-request Full/Light attribution is not supplied by these new logs.
